...
const LegalActCard = ({ item, deleteItem ,index,toggleRemoveModal}) => {
  ...
  return (
    ...
    <>
      ...
      <tr className="hover:bg-gray-50 cursor-pointer" key={index} >
        ...
        <td className="p-3">
          <p className="text-gray-700">{item.title[`${i18n.language}`]}</p>
          {/* <p className="text-gray-700">{item}</p> */}
        </td>
        
        <td className="p-3">
          <p className="text-gray-700">{item.description[`${i18n.language}`]}</p>
        </td>
        <td className="p-3 text-right">
          <div className="flex justify-end">
          <button
              onClick={() => {history.push(`/adalat-edaralary/${item?.id}`)}}
            ...
            <NavLink
              to={`/adalat-edaralary/edit/${item?.id}`}
