...
import { deleteLawFile, getLaw } from "../../redux/actions/lawsAction";
...
const LawShow = () => {
  ...
  const law = useSelector((state) => state.laws.law);
  const law_files = useSelector((state) => state.laws.law.law_files);
  const [deleteConfirm, setDeleteConfirm] = useState([]);

  const deleting = () => {
    dispatch(deleteLawFile(deleteConfirm.id));
    setDeleteConfirm({ delete: false, id: null });
  };
  const handleDeleting = (confirmation, law_file_id) => {
    setDeleteConfirm({ delete: confirmation, id: law_file_id });
  };
            ...
            <div className="flex items-center">
              ...
              <div className="flex flex-col">
                <h1 className="text-base lg:text-xl font-bold font-montserrat-bold text-gray-800">
                  {law && law.title && getByLocale(law.title)}
                </h1>
                <small className="flex font-montserrat-medium text-base text-gray-500">
                  <p>{t("total")}:</p>
                  <p className="ml-2">{law && law.law_files_count}</p>
                </small>
              </div>
            </div>

            <NavLink
              to={`/law/category/${id}/add`}
          ...
          <main className="bg-white rounded-xl lg:px-5 my-5">
            {law_files.map((law_file, key) => {
              return (
                ...
                >
                  <NavLink to={`/law/file/${law_file.id}`}>
                    <p className="text-gray-800">
                      {getByLocale(law_file.title)}
                    </p>
                  </NavLink>

                  <Collapse className="w-56 text-sm">
                    <NavLink
                      to={`/lawfile/${law_file.id}/files`}
                      className="flex hover:bg-gray-100 duration-300 text-left border-b text-green-700 w-full px-5 py-2"
                    >
                      <IoDocumentAttachOutline className="mr-2" size={20} />
                      {t("law_files")}
                    </NavLink>

                    <NavLink
                      to={`/law/file/${law_file.id}/edit`}
                      className="flex hover:bg-gray-100 duration-300 text-left border-b text-green-700 w-full px-5 py-2"
                    >
                    ...
                    <NavLink
                      to={`/law/file/${law_file.id}/edit_text`}
                      className="flex hover:bg-gray-100 duration-300 text-left border-b text-green-700 w-full px-5 py-2"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handleDeleting(true, law_file.id)}
