...
import { putLawCategory } from "../../redux/actions/lawsAction";
import { setLoading } from "../../redux/reducers/mainReducer";
import Modal from "../../components/Modal/Modal";

const LawsEdit = ({ law, editConfirm, close }) => {
  ...
  const onSubmit = (e) => {
    ...
    dispatch(putLawCategory(data));
    close();
    dispatch(setLoading(true));
  };

  return (
    <Modal isOpen={editConfirm} close={close}>
      <form
        onSubmit={(e) => onSubmit(e)}
        className="font-montserrat-medium p-5 lg:p-8"
      >
        <h1 className="text-2xl font-montserrat-bold">
          {" "}
          Hukuk bölümi üýtgetmek{" "}
        </h1>
        ...
        <aside className="flex flex-col my-4">
          ...
          <input
            ...
            required
            onChange={(e) => {
              setTitle({ ...title, tm: e.target.value });
            }}
            placeholder="Täzelik sözbaşyny giriziň"
            value={title && title.tm && title.tm}
