...
import LegalActCard from "./LegalActCard";
...
const LegalActs = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await api.get('regulatory_legal_acts',{
    ...
    const remove = async(id) => {
        if(id){
            try{
                const response = await api.delete(`regulatory_legal_acts/${id}`)
                if (response.status ===200){
            ...
            <AppLayout>
                ...
                <section>
                    <aside className="bg-white p-3 rounded-lg flex items-center justify-between">
                        ...
                        <button
                            onClick={() => {history.push('/legalActs/add') }}
                            ...
                            <table className="w-full align-middle">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="bg-gray-200 text-left px-3 py-2">Namanyň ady</th>
                                ...
                                {data.map((act, index) => {
                                    // debugger
                                    return (
                                        <LegalActCard key={index} index={index} act={act} toggleRemoveModal={toggleRemoveModal} />
                                    );
                                })}
                            </table>
                        </main>
